...
void fetch_edgeS(
        ...
        bool normal) {
    if (column_found(info[0].colNumber)) {
        edge->gid = pgr_SPI_getText(tuple, tupdesc, info[0]);
    } else {
        edge->gid = *default_id;
        ++(*default_id);
    }
    ...
    if (normal) {
        edge->gid1 = pgr_SPI_getText(tuple, tupdesc,  info[1]);
        edge->gid2 = pgr_SPI_getText(tuple, tupdesc, info[2]);
    } else {
        edge->gid1 = pgr_SPI_getText(tuple, tupdesc,  info[1]);
        edge->gid2 = pgr_SPI_getText(tuple, tupdesc, info[2]);
    }

    edge->gid1_type = pgr_SPI_getText(tuple, tupdesc, info[3]);
    //edge->gid1_type = SPI_getvalue(*tuple, *tupdesc, info.colNumber);
    //SPI_getvalue(*tuple, *tupdesc, info.colNumber)

    if (column_found(info[4].colNumber)) {
        edge->gid2_type = pgr_SPI_getText(tuple, tupdesc, info[4]);
    } else {
        edge->gid2_type = NULL;
    }

    *valid_edges = edge->gid1_type < 0? *valid_edges: *valid_edges + 1;
    *valid_edges = edge->gid2_type < 0? *valid_edges: *valid_edges + 1;
}
...
get_edges_51_columns(
    ...
    info[1].name = "gid1";
    info[2].name = "gid2";
    info[3].name = "gid1_type";
    info[4].name = "gid2_type";
    ...
    while (moredata == true) {
        ...
        if (ntuples > 0) {
            if ((*edges) == NULL)
                (*edges) = (pgr_edge_ts *)
                palloc0(total_tuples * sizeof(pgr_edge_ts));
            else
                (*edges) = (pgr_edge_ts *)
                repalloc((*edges), total_tuples * sizeof(pgr_edge_ts));
            ...
            for (t = 0; t < ntuples; t++) {
                ...
                fetch_edgeS(&tuple, &tupdesc, info,
                    &default_id, 
...
/* select gid, gid1, gid2, gid1_type, gid2_type */
void
pgr_get_gedges(
    char *edges_sql,
    pgr_edge_ts **edges,
    ...
    bool ignore_id = false;
    bool normal = true;
    get_edges_51_columns(edges_sql, edges, total_edges, ignore_id, normal);
}
